    ...
    <div class="middle-box">
      <div>
        ...
        <div class="step well">
            ...
            <ul class="jobsList">
              @foreach ($educations as $education)
                @php
                  $started_year = strtotime($education->start_date);
                  $started_year = date('Y',$started_year);
                  $ended_year = strtotime($education->end_date);
                  $ended_year = date('Y',$ended_year);
                @endphp
                <li>
                  <a href="{{route('employee_resume.build.education.edit', ['education' => $education->id])}}">
                    <h4>{{$education->school}}</h4>
                    <p class="hintText  mb-0">{{$education->degree}}</p>
                    <p class="hintText">{{$started_year}} - {{$ended_year}}</p>
                  </a>

                  <a href="{{route('employee_resume.build.education.edit', ['education' => $education->id])}}" class="ml-auto">
            ...
            <a class="uploadCv mt-4 mb-4" href="{{route('employee_resume.build.education.create')}}">
        ...
        <div class="text-center">
          <a href="{{route('employee_resume.build.certification')}}" style="color: #fff !important;"
          class="action next btn blue-btn ctrl-standard is-reversed typ-subhed fx-sliderIn btn-block">Next</a>
          <a href="{{route('employee_resume.build.work_history')}}" class="action back btn btn-info mt-3" style="font-size: 12px;">Back</a>
        </div>
...
<div class="container">
  ...
  <div class="row mt-5">
    ...
    <div class="col-md-12 mb-3 pb-3 border-bottom">
      <a href="{{route('employee_resume.build.education.edit', ['education' => $education->id])}}">
        <h5 class="text-primary">{{$education->school}}</h5>
    ...
    <div class="col-md-12">
      <a href="{{route('employee_resume.build.education.create')}}" class="btn btn-primary">{{ __('Add more Educations') }}</a>
      <a href="#" class="btn btn-secondary">{{ __('Next') }}</a>
    </div>
  </div>
</div> --}}
@endsection
