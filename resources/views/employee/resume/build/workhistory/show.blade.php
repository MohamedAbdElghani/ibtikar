      ...
      <div>
        <div class="progress">
          <div class="progress-bar progress-bar-info progress-bar-striped active" role="progressbar"
              aria-valuemin="0" aria-valuemax="43" style="width: 43%;">
              <span>43%</span>
          </div>
        </div>
        ...
        <div class="step well">
          ...
          <ul class="jobsList">
            @foreach ($work_histories as $work)
              ...
              @endphp
              <li>
                <a href="{{route('employee_resume.build.work_history.edit', ['work' => $work->id])}}">
                  <h4>{{$work->company}}</h4>
                  <p class="blueText mb-0">{{$work->title}}</p>
                  <p class="hintText mb-0">{{$started_year}} - 
                    @if($work->currently_work_here) <span class="h5">Present</span> @else {{$ended_year}} @endif</p>
                  <p class="hintText">{{$work->accomplishment}}</p>
                </a>
                <a href="{{route('employee_resume.build.work_history.edit', ['work' => $work->id])}}" class="ml-auto">
          ...
          <a class="uploadCv mt-4 mb-4" href="{{route('employee_resume.build.work_history.create')}}">
        ...
        <div class="text-center">
          <a href="{{route('employee_resume.build.education')}}" style="color: #fff !important;"
          class="action next btn blue-btn ctrl-standard is-reversed typ-subhed fx-sliderIn btn-block">Next</a>
          <a href="{{route('employee_resume.build.primary_role_experience')}}" class="action back btn btn-info mt-3" style="font-size: 12px;">Back</a>
        </div>
      </div>
    </div>
  </div>
...
{{-- <div class="container">
  ...
  <p>We'll use your skills and experience to recommend a salary.</p>
  ...
  <div class="row mt-5">
    ...
    <div class="col-md-12 mb-3 pb-3 border-bottom">
      <a href="{{route('employee_resume.build.work_history.edit', ['work' => $work->id])}}">
        ...
        <p>{{$work->company}}</p>
        <p>{{$work->started_month.' '.$work->started_year}} - {{$work->currently_work_here ? 'current' : $work->ended_month.' '.$work->ended_year}} </p>
      </a>
    </div>
    @endforeach

    <div class="col-md-12">
      <a href="{{route('employee_resume.build.work_history.create')}}" class="btn btn-primary">{{ __('Add more experience') }}</a>
      <a href="{{route('employee_resume.build.education')}}" class="btn btn-secondary">{{ __('Next') }}</a>
    </div>
  </div>
</div> --}}
@endsection
